...
| **opencode** | UNVERIFIED | `.opencode/plugins/pabel.js` (a JS bridge, not a config entry) + `opencode.json` for MCP | `~/.config/opencode/plugins/pabel.js` + `~/.config/opencode/opencode.json` | `throw` from `tool.execute.before` (no allow/deny field exists) | Folded into the thrown message - **whether that message reaches the model is unconfirmed upstream** (see `docs/known-gaps.md`) | Yes (`opencode.json`) | No |
| **Cline** | NO ADAPTER | - | - | - | - | - | - |
