...
- **Status** - `VERIFIED` (confirmed end-to-end against a real install),
  ...
  `NO ADAPTER` (documented gap, see `docs/known-gaps.md`).
...
| **Gemini CLI** | UNVERIFIED | `.gemini/settings.json` | `~/.gemini/settings.json` | `{"decision": "deny", "reason"}` | Folded into `reason` (`additionalContext` belongs to a different, turn-scoped event) | Not wired yet | No |
| **OpenAI Codex CLI** | NO ADAPTER | - | - | - | - | - | - |
| **Cline** | NO ADAPTER | - | - | - | - | - | - |
...
- **Gemini CLI** - MCP tool-naming (`mcp_<server>_<tool>`) assumes the
  server name itself has no underscore (true for "pabel").
- **Codex CLI / Cline / Continue.dev** - see `docs/known-gaps.md` for why
  each has no adapter at all. All three can still be pointed at the
  deployed PABEL server directly as a normal MCP client (`whoami`/
  `read_document` become callable tools) - there's just no enforcement,
  no blocking of a direct `.abe` read.
