...
async def _call_read_document(server_url, token, content_b64, name):
    async with streamablehttp_client(
        ...
        async with ClientSession(read_stream, write_stream) as mcp_session:
            ...
            result = await mcp_session.call_tool(
                "read_document", {"content": content_b64, "name": name})
    if result.isError:
        text = "; ".join(
...
def read_document(path, name):
    ...
    deployed PABEL server's read_document tool. Raises RelayError/AuthError
    on failure - callers turn that into a denial message."""
    ...
    try:
        return anyio.run(_call_read_document, server_url, token, content_b64, name)
    except AuthError:
        raise
